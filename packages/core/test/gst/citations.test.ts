...
describe('CGST_ACT_SECTIONS registry', () => {
  ...
  it('Section 50 interest variants distinguish the 50(1) and 50(3) bases', () => {
...
describe('CBIC_NOTIFICATIONS registry', () => {
  ...
  it('GST 2.0 rationalised rate schedule (9/2025-CT (Rate)) is present', () => {
...
describe('GST_COUNCIL_MEETINGS registry (via the citations barrel)', () => {
  it('every entry is a gst-council-meeting citation with a note', () => {
    for (const [key, entry] of Object.entries(GST_COUNCIL_MEETINGS)) {
      expect(entry.kind).toBe('gst-council-meeting');
      expect(entry.note, `${key} missing note`).toBeDefined();
    }
  });

  it('the 56th meeting (GST 2.0 rate rationalisation) is present', () => {
