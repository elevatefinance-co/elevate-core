...
describe('ITA_SECTIONS registry', () => {
  ...
  it('matches the exact registered shape for SEC_194I_A (clause + note, subSection omitted)', () => {
    expect(ITA_SECTIONS.SEC_194I_A).toEqual({
      ...
      section: '194I',
      clause: 'a',
      note: 'TDS on rent of plant / machinery / equipment -- 2 percent above Rs 2.4 lakh',
    });
...
describe('FINANCE_ACTS_TDS registry', () => {
  ...
  it('Oct 2024 cliff coverage -- 7 rate reductions + 194T introduction + 194F repeal', () => {
    expect(FINANCE_ACTS_TDS.FA_2024_OCT_CLIFF_194D.year).toBe(2024);
